...
if (!class_exists('BingMapsSettingsAdminListClass')) 
{
	...
	{		
		...
		function GetDetailsRowsDefinition()
		{
			...
			// Locale information is defined at https://msdn.microsoft.com/en-us/library/gg427600.aspx
...
include BINGMAPS_INCLUDE_PATH.'bingmapslib_settingsadmin.php';
...
if (!class_exists('BingMapsSettingsAdminClass')) 
{
	...
	{		
		...
		function ProcessActionButtons()
		{
			...
			if (BingMapsLibUtilsClass::IsElementSet('post', 'savebutton'))
			{
				$this->CheckAdminReferer(plugin_basename($this->caller)); // check nonce created by wp_nonce_field()
				...
				if ($SettingsUpdateMsg === '')
				{
					...
					BingMapsLibEscapingClass::Safe_EchoHTML('<div id="message" class="bingmaps-updated">'.__('Settings have been saved.', 'bingmaps').'</div>');
				}
				else
				{
					BingMapsLibEscapingClass::Safe_EchoHTML('<div id="message" class="bingmaps-error">'.$SettingsUpdateMsg.'</div>');
					BingMapsLibEscapingClass::Safe_EchoHTML('<div id="message" class="bingmaps-error">'.__('Settings have NOT been saved.', 'bingmaps').'</div>');
				}
			}
		}
