...
if (!class_exists('BingMapsLibEscapingClass')) 
{

	class BingMapsLibEscapingClass  // Define class
	{
		...
		static function Safe_EchoHTML($echoData)
		{
			...
			$htmlTags = BingMapsLibUtilsClass::GetAllowedHTMLTags();
			$protocols = BingMapsLibUtilsClass::GetAllowedProtocols();
		...
		static function Safe_EchoScript($js)
		{
			...
			if (strpos($js, '</script') !== false)
			{
				...
				BingMapsLibEscapingClass::Safe_EchoHTML(self::$jsBuffer);
				self::$jsBuffer = '';
				self::$inJS = false;
			}
		}
