...
if (!class_exists('BingMapsLibAdminListClass')) 
{
	...
	{		
		...
		function __construct($env, $editMode /* = false */, $newTableType = self::TABLETYPE_HTML) //constructor
		{
			...
			{
				if (!$this->editMode)
					$this->hiddenRowsButtonId = __('Details', 'bingmaps');
				else
				{
					$this->hiddenRowClass = '';
		...
		static function Output_AjaxGlobals()
		{
			...
			$pageId = BingMapsLibUtilsClass::GetHTTPTextElem('get', 'page');
			$js_ajaxvars = "<script>\n";
				
			$js_ajaxvars .= 'var BingMapsLib_Ajax_NOnce = "'.wp_create_nonce(BINGMAPSLIB_AJAXNONCEKEY).'"'.";\n";
			$js_ajaxvars .= 'var BingMapsLib_Ajax_PluginName = "'.BINGMAPSLIB_PLUGINNAME.'"'.";\n";
			$js_ajaxvars .= 'var BingMapsLib_Ajax_DebugElemId = "'.BINGMAPSLIB_PLUGINNAME.'-ajax-debug"'.";\n";
			$js_ajaxvars .= 'var BingMapsLib_Ajax_PageId = "'.$pageId.'"'.";\n";

			$js_ajaxvars .= "</script>\n";

			BingMapsLibEscapingClass::Safe_EchoScript($js_ajaxvars);
		...
		{
			BingMapsLibUtilsClass::UndefinedFuncCallError($this, 'GetDBFilterCounts');
		}
		...
		static function OutputFilterLinks($filterRowDefs, $defaultFilterId = '')
		{
			...
			$rowFilter = BingMapsLibUtilsClass::GetHTTPTextElem('get', 'filter', $defaultFilterId); 
			...
			$current_url = BingMapsLibUtilsClass::GetPageURL();
			$current_url = remove_query_arg( 'filter', $current_url);
			$current_url = remove_query_arg( 'paged', $current_url);
			$current_url = remove_query_arg( 'searchsalestext', $current_url);
				
			$searchText = BingMapsLibUtilsClass::GetHTTPTextElem('request', 'searchsalestext');
			if ($searchText != '')
			...
			foreach ($filterRowDefs as $filterId => $filterRowDef)
			{
				...
				$filterClass = BingMapsLibMigratePHPClass::Safe_str_replace(' ', '', $filterId);
				$hrefClass = ($rowFilter == $filterId) ? 'current' : '';
				$filterClass = 'filter-'.BingMapsLibMigratePHPClass::Safe_strtolower($filterClass);
...
/*				
			BingMapsLibEscapingClass::Safe_EchoHTML("<div class=filter-links>\n");
			BingMapsLibEscapingClass::Safe_EchoHTML($html_filterLinks);
...
<ul class=subsubsub>{$html_filterLinks}</ul>
		...
		{
			if (isset($settingOption[BingMapsLibTableClass::TABLEPARAM_ITEMS]))
			{
				$selectOpts = $settingOption[BingMapsLibTableClass::TABLEPARAM_ITEMS];
			}
			else if (isset($settingOption[BingMapsLibTableClass::TABLEPARAM_DIR]))
			{
				if (isset($settingOption[BingMapsLibTableClass::TABLEPARAM_EXTN]))
					$fileExtns = $settingOption[BingMapsLibTableClass::TABLEPARAM_EXTN];
				else
					$fileExtns = '*';
				$shortenBy = isset($settingOption[BingMapsLibTableClass::TABLEPARAM_HIDEEXTNS]) ? $shortenBy = BingMapsLibMigratePHPClass::Safe_strlen($fileExtns)+1 : 0;
				...
				foreach($fileExtnsArray as $fileExtn)
				{
					...
					if (BingMapsLibMigratePHPClass::Safe_substr($dir, BingMapsLibMigratePHPClass::Safe_strlen($dir)-1, 1) != '/')
						$dir .= '/';
					...
					foreach ($filesList as $key => $path)
					{
						...
						$selectText = ($shortenBy > 0) ? BingMapsLibMigratePHPClass::Safe_substr($selectValue, 0, BingMapsLibMigratePHPClass::Safe_strlen($selectValue) - $shortenBy) : $selectValue;
						$selectOpts[] = "$selectValue|$selectText";
					}
				}
			}
			else if (isset($settingOption[BingMapsLibTableClass::TABLEPARAM_FUNC]))
			{
				$functionId = $settingOption[BingMapsLibTableClass::TABLEPARAM_FUNC];
			...
			if (isset($settingOption[BingMapsLibTableClass::TABLEPARAM_ADDEMPTY]))
			{
				$selectOptsArray[''] = '';
			}
			...
			foreach ($selectOpts as $selectOpt)
			{
				...
				else
				{
					...
					$selectOptText = __($selectAttrs[1], 'bingmaps');
				}
		...
		function GetHTMLTag($settingOption, $controlValue, $editMode = true, $class = '')
		{
			...
			if ($this->tableUsesSerializedPost)
			{
				switch($settingType)
				{
					...
					case self::TABLEENTRY_DATETIME:
						...
						$class .= ' bingmapslib_PostVal '.$class;
						break;
				}
			}
			...
			{
				$funcName = $columnDef[BingMapsLibTableClass::TABLEPARAM_CANEDIT];
				$editMode = $this->$funcName($result);
			}
			
			if (!$editMode)
			{
				if ($this->allowHiddenTags) BingMapsLibEscapingClass::Safe_EchoHTML('<input type="hidden" '.$controlIdDef.' value="'.$controlValue.'"/>'."\n");
				...
				switch ($settingType)
				{
					...
					case self::TABLEENTRY_CHECKBOX:
						$controlValue = $controlValue ? __('Yes', 'bingmaps') : __('No', 'bingmaps');
						$settingType = self::TABLEENTRY_VIEW;
						break;		
			...
			switch ($settingType)
			{
				case self::TABLEENTRY_DATETIME:
					...
					$eventHandler = " readonly=true onclick=\"javascript:BingMapsLib_DateModeCalendarSelector(this, '".$this->dateTimeMode."', '".$dateMode."')\" ";
					$editControl  = '<input type="text"'.$eventHandler.' size="'.$editSize.'" '.$controlIdDef.' value="'.$controlValue.'" />'."\n";
					if ($this->allowHiddenTags) $editControl .= '<input type="hidden" '.BingMapsLibMigratePHPClass::Safe_str_replace('="', '="curr', $controlIdDef).' value="'.$controlValue.'" />'."\n";
				...
				case self::TABLEENTRY_FLOAT:
					...
					if ($settingType == self::TABLEENTRY_INTEGER)
						...
						$limits .= ", true";
					$eventHandler = ' onkeypress="BingMapsLib_OnKeypressNumericOnly(this, event, '.$limits.');" ';
					$eventHandler .= ' onchange="BingMapsLib_OnChangeNumericOnly(this, event, '.$limits.');" ';
					// Drop into next case ...
				
				case self::TABLEENTRY_TEXT:
					$controlValue = BingMapsLibMigratePHPClass::Safe_htmlspecialchars($controlValue);
					// Drop into next case ...
				...
				case self::TABLEENTRY_COOKIE:
					...
					{
						//$eventHandler = ' onkeypress="BingMapsLib_OnKeypressNumericOnly(this, event, '.$limits.');" ';
						$eventHandler .= ' onchange="BingMapsLib_OnChangeText(this, event);" ';
					}
					...
					if ($this->allowHiddenTags) $editControl .= '<input type="hidden" '.BingMapsLibMigratePHPClass::Safe_str_replace('="', '="curr', $controlIdDef).' value="'.$controlValue.'" />'."\n";
					if (isset($settingOption[BingMapsLibTableClass::TABLEPARAM_TEXT])) 
						$editControl .=  __($settingOption[BingMapsLibTableClass::TABLEPARAM_TEXT], 'bingmaps');
					break;

				case self::TABLEENTRY_TEXTBOX:
					$controlIdDef .= ' class="'.$class.'" ';
					$eventHandler = ($onChange == '') ? 'onchange="BingMapsLib_OnChangeTextBox(this, event);" ' : $onChange;
					if ($this->inputElemClass != '') $eventHandler .= ' class="'.$this->inputElemClass.'" ';
				...
				case self::TABLEENTRY_SELECT:
					...
					$controlIdDef .= ' class="'.$class.'" ';
					$eventHandler = ($onChange == '') ? ' onchange="BingMapsLib_OnChangeSelect(this, event);" ' : $onChange;
					if ($this->inputElemClass != '') $eventHandler .= ' class="'.$this->inputElemClass.'" ';
					$selectOptsArray = self::GetSelectOptsArray($settingOption);
				...
				case self::TABLEENTRY_CHECKBOX:
					$controlIdDef .= ' class="'.$class.'"';
					$eventHandler = ($onChange == '') ? 'onchange="BingMapsLib_OnChangeCheckbox(this, event);" ' : $onChange;
					if ($this->inputElemClass != '') $eventHandler .= ' class="'.$this->inputElemClass.'" ';
					$checked = ($controlValue == true) ? ' checked="yes"' : '';
					$cbText = __($settingOption[BingMapsLibTableClass::TABLEPARAM_TEXT], 'bingmaps');
			...
			$enableButton = current_user_can(BINGMAPSLIB_CAPABILITY_SETUPUSER);
			if (isset($settingOption[self::TABLEPARAM_BUTTON]))
			{
				...
				if ($enableButton)
				{
					...
					if ($this->tableUsesSerializedPost)
					{
						$clickEvent="return bingmapslib_JSONEncodePost(this, 'bingmapslib_PostVal')";
				...
				$buttonText = __($settingOption[self::TABLEPARAM_BUTTON], 'bingmaps');
			...
			foreach ($this->columnDefs as $key => $columnDef)
			{
				...
				switch ($columnDef[self::TABLEPARAM_TYPE])
				{
					...
					default:
						...
						BingMapsLibEscapingClass::Safe_EchoHTML("Can't display this table - Label:".$columnDef[self::TABLEPARAM_LABEL]." Id:".$columnDef[self::TABLEPARAM_ID]." Column Type:".$columnDef[self::TABLEPARAM_TYPE]."<br>\n");
						break 2;
				}
			}
			...
			if ($canDisplayTable)
			{
				...
				foreach ($this->columnDefs as $columnDef)
				{
					...
					if (isset($columnDef[self::TABLEPARAM_ID]))
					{
						...
						if ($this->updateFailed && BingMapsLibUtilsClass::IsElementSet('post', $columnId.$recId))
						{
							if ( (isset($columnDef[self::TABLEPARAM_TYPE])) 
							  && ($columnDef[self::TABLEPARAM_TYPE] == self::TABLEENTRY_TEXTBOX) )
							{
								// Error updating values - Get value(s) from form controls
								$currVal = BingMapsLibUtilsClass::GetHTTPTextareaElem('post', $columnId.$recId);							
							}
							else
							{
								// Error updating values - Get value(s) from form controls
								$currVal = BingMapsLibUtilsClass::GetHTTPTextElem('post', $columnId.$recId);									
							}
						}
						else
						{
					...
					if (isset($columnDef[self::TABLEPARAM_DECODE]))
					{
						...
						if (BingMapsLibMigratePHPClass::Safe_strpos($currVal, '>'))
						{
					...
					if ((!$this->editMode) && ($columnType != self::TABLEENTRY_FUNCTION))
					{
						...
						{
							case self::TABLEENTRY_CHECKBOX:
								$currVal = ($currVal == 1) ? __('Yes', 'bingmaps') : __('No', 'bingmaps');
								break;
						...
						if (isset($columnDef[self::TABLEPARAM_CANEDIT]))
						{
							...
							if (!$editMode)
							{
								if ($columnType == self::TABLEENTRY_SELECT)
								{
									...
									$srchText = $currVal.'|';
									$srchLen = BingMapsLibMigratePHPClass::Safe_strlen($srchText);
									foreach ($columnDef[BingMapsLibTableClass::TABLEPARAM_ITEMS] as $item)
									{
										if (BingMapsLibMigratePHPClass::Safe_substr($item, 0, $srchLen) === $srchText)
										{
											$currVal = BingMapsLibMigratePHPClass::Safe_substr($item, $srchLen);
											break;
										}
									}
								}
					...
					switch ($columnType)
					{
						...
						case self::TABLEENTRY_COOKIE:
							...
							if (BingMapsLibUtilsClass::IsElementSet('cookie', $cookieID))
								$currVal = BingMapsLibUtilsClass::GetHTTPTextElem('cookie', $cookieID);
							else
								$currVal = '';
							// Fall into next case ...
							
						case self::TABLEENTRY_TEXTBOX:
							$currVal = BingMapsLibMigratePHPClass::Safe_htmlspecialchars($currVal);
						...
						case self::TABLEENTRY_FLOAT:
							...
							{
								BingMapsLibEscapingClass::Safe_EchoHTML("No Len entry in Column Definition<br>\n");
							}
						...
						case self::TABLEENTRY_DATETIME:
							$size = 28;
							$inputClass = 'bingmaps'.'-dateinput';
							$dateMode = isset($columnDef[self::TABLEPARAM_DATEMODE]) ? $columnDef[self::TABLEPARAM_DATEMODE] : 'future';
							$extraParams = "class=\"".$inputClass."\" readonly=true onclick=\"javascript:BingMapsLib_DateModeCalendarSelector(this, '".$this->dateTimeMode."', '".$dateMode."')\" ";
						...
						case self::TABLEENTRY_READONLY:
							...
							{
								$hiddenTag = '<input type="hidden" class="bingmapslib_PostVal" id="'.$columnId.$recId.'" value="'.$hiddenVal.'">';
							}
							else
							{
							...
							if (isset($columnDef[BingMapsLibTableClass::TABLEPARAM_LINK]))
							{
								...
								if (isset($columnDef[BingMapsLibTableClass::TABLEPARAM_LINKURL]))
								{
									$optionId = $columnDef[self::TABLEPARAM_ID];
									$funcName = $columnDef[BingMapsLibTableClass::TABLEPARAM_LINKURL];
									$currValLink = $this->$funcName($result->$optionId, $result);
									$target = 'target="_blank"';
								}
								else if (isset($columnDef[BingMapsLibTableClass::TABLEPARAM_LINKTO]))
								{
									$currValLink .= "http://";		// Make link absolute
									$currValLink .= $result->$columnDef[BingMapsLibTableClass::TABLEPARAM_LINKTO];
									$target = 'target="_blank"';
								}
								else
								{
		...
		function GetCurrentOptionValue($option, $optionId, $result)
		{
			if (isset($option[self::TABLEPARAM_TYPE]) && ($option[self::TABLEPARAM_TYPE] != self::TABLEENTRY_COOKIE))
			{
				...
				if (isset($option[BingMapsLibTableClass::TABLEPARAM_DECODE]))
				{
					$funcName = $option[BingMapsLibTableClass::TABLEPARAM_DECODE];
					$currVal = $this->$funcName($currVal, $result);
				}
			}
			else if (BingMapsLibUtilsClass::IsElementSet('cookie', $optionId))
				$currVal = BingMapsLibUtilsClass::GetHTTPAlphaNumericElem('cookie', $optionId); 
			else
				$currVal = '';
			...
			if (count($this->detailsRowsDef) > 0)
			{
				...
				foreach ($this->detailsRowsDef as $option)
				{
					...
					if (isset($option[self::TABLEPARAM_LABEL]))
						$optionLabel = __($option[self::TABLEPARAM_LABEL], 'bingmaps');
					...
					switch ($option[self::TABLEPARAM_TYPE])
					{
						...
						default:
							...
							if (!$nextInline)
								$hiddenRows .= $tableRowTag."\n";
							if (BingMapsLibMigratePHPClass::Safe_strlen($option[self::TABLEPARAM_LABEL]) > 0)
							{
								if (!$nextInline)
		...
		function OutputJSDateConstants()
		{
			...
			BingMapsLibEscapingClass::Safe_EchoScript("<script type='text/javascript'>\n");	
		
			// Use a date for a Sunday (22/12/2013)			
			$scriptOutput = "var bingmapslib_WeekDayName1 = [";
			for ($dayNo = 22, $i=1; $i <= 7; $dayNo++, $i++)
			{
				$day = date_i18n("l",BingMapsLibMigratePHPClass::Safe_strtotime('2013-12-'.$dayNo));
				if ($i > 1) $scriptOutput .= ', ';
			...
			$scriptOutput .= "var bingmapslib_WeekDayName2 = [";
			...
			$scriptOutput .= "var bingmapslib_MonthName = [";
			for ($monthNo = 1; $monthNo <= 12; $monthNo++)
			{
				$month = date_i18n("F",BingMapsLibMigratePHPClass::Safe_strtotime('2000-'.$monthNo.'-20'));
				if ($monthNo > 1) $scriptOutput .= ', ';
				$scriptOutput .= '"'.ucwords($month).'"';
			}
			$scriptOutput .= "];\n";
								
			$scriptOutput .= "var bingmapslib_textOK = \"".__("OK", 'bingmaps')."\";\n";
			$scriptOutput .= "var bingmapslib_textCancel = \"".__("Cancel", 'bingmaps')."\";\n";
	
			BingMapsLibEscapingClass::Safe_EchoScript($scriptOutput);
			
			BingMapsLibEscapingClass::Safe_EchoScript("</script>\n");
		...
		function OutputJavascript($selectedTabIndex = 0)
		{
			...
			$lastTabId = BingMapsLibUtilsClass::GetHTTPAlphaNumericElem('post', 'lastTabId', ""); 
			BingMapsLibEscapingClass::Safe_EchoHTML('<input type="hidden" name="lastTabId" id="lastTabId" value="'.$lastTabId.'"/>'."\n");
			...
			BingMapsLibEscapingClass::Safe_EchoScript($js);
		}

		function GetLimitSQL()
		{
			$rowsPerPage = $this->myDBaseObj->GetRowsPerPage();
			$currentPage = BingMapsLibTableUtilsClass::GetCurrentPage();	
		...
		function OutputList($results, $updateFailed = false)
		{
			...
			if ($this->totalRows == 0) 
			{
				...
				$this->tableTags = BingMapsLibMigratePHPClass::Safe_str_replace('class="', 'class="'.$this->blankTableClass.' ', $this->tableTags);
			}
			else
			{
			...
			foreach ($this->columnDefs as $column)
			{
				...
				$columnLabel = __($column[self::TABLEPARAM_LABEL], 'bingmaps');
				...
				foreach($results as $result)
				{
					...
					if (!$this->AddResultFromTable($result))
					{
						if (!isset($this->usedAddResult))
						{
							...
							BingMapsLibEscapingClass::Safe_EchoHTML("<br>Error returned by AddResultFromTable function in ".get_class($this)." class<br>\n");
...
var defaultTabIndex = ".$defaultTab.";
	...
	class BingMapsLibAdminDetailsListClass extends BingMapsLibAdminListClass
	{
	}
}
