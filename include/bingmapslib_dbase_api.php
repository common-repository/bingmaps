...
require_once BINGMAPSLIB_INCLUDE_PATH.'bingmapslib_utils.php';
require_once BINGMAPSLIB_INCLUDE_PATH.'bingmapslib_dbase_base.php';
require_once BINGMAPSLIB_INCLUDE_PATH.'bingmapslib_logfile.php';
...
if (!class_exists('BingMapsLibDBaseClass'))
{
	...
	if (!defined('BINGMAPSLIB_SESSION_TIMEOUT'))
		define('BINGMAPSLIB_SESSION_TIMEOUT', 60*60*24);

	define('BINGMAPSLIB_PLUGINNAME', basename(dirname(dirname(__FILE__))));
	define('BINGMAPSLIB_FILENAME_COMMSLOG', BINGMAPSLIB_PLUGINNAME.'.log');

	if (!defined('BINGMAPSLIB_SITEID_OPTIONID'))
		define('BINGMAPSLIB_SITEID_OPTIONID', 'OrganisationID');

	if (!defined('BINGMAPSLIB_LOGOIMAGE_OPTIONID'))
		define('BINGMAPSLIB_LOGOIMAGE_OPTIONID', 'PayPalLogoImageFile');

	class BingMapsLibDBaseClass extends BingMapsLibGenericDBaseClass // Define class
	{
		...
		const SessionDebugPrefix = 'bingmapslib_debug_';
		...
		var	$emailObjClass = 'BingMapsLibHTMLEMailAPIClass';
		var	$emailClassFilePath = BINGMAPSLIB_INCLUDE_PATH.'bingmapslib_htmlemail_api.php';
		...
		{
			if (current_user_can(BINGMAPSLIB_CAPABILITY_DEVUSER))
			{
				$className = get_class($this);
				BingMapsLibEscapingClass::Safe_EchoHTML("PurgeDB not defined in $className class <br>\n");
			}
		}
		...
		{
			$this->UserCapToServervar(BINGMAPSLIB_CAPABILITY_SYSADMIN);
			$this->SetSessionElem(BINGMAPSLIB_SESSION_VALIDATOR, true);	// Set Element and Write Cache to DB
		}
		...
		static function GetIPAddr()
		{
			...
			if (BingMapsLibUtilsClass::IsElementSet('server', 'X-Forwarded-For'))
			{
				$the_ip  = BingMapsLibUtilsClass::GetHTTPTextElem('server', 'X-Forwarded-For');
			}
			elseif (BingMapsLibUtilsClass::IsElementSet('server', 'HTTP_X_FORWARDED_FOR'))
			{
				$the_ip  = BingMapsLibUtilsClass::GetHTTPTextElem('server', 'HTTP_X_FORWARDED_FOR');
			}
			else
			{
				$the_ip  = BingMapsLibUtilsClass::GetHTTPTextElem('server', 'REMOTE_ADDR');
			}
		...
		function IsButtonClicked($buttonID)
		{
			...
			$rtnVal = (BingMapsLibUtilsClass::IsElementSet('request', $normButtonID) || BingMapsLibUtilsClass::IsElementSet('request', $normButtonID.'_x'));
			return $rtnVal;
		}
		...
		function getDebugFlagsArray()
		{
			...
			$len = BingMapsLibMigratePHPClass::Safe_strlen(self::SessionDebugPrefix);
			$sessionKeys = array_keys($this->GetSession());
			foreach ($sessionKeys as $key)
			{
				if (BingMapsLibMigratePHPClass::Safe_substr($key, 0, $len) != self::SessionDebugPrefix)
					continue;
				$debugFlagsArray[] = $key;
			}
		...
		function WPNonceField($referer = '', $name = '_wpnonce', $echoOut = true)
		{
			...
			if ($echoOut) BingMapsLibEscapingClass::Safe_EchoHTML($html_nonce);
			return $html_nonce;
		}
		...
		function ActionButtonHTML($buttonText, $caller, $domainId, $buttonClass, $elementId = 0, $buttonAction = '', $extraParams = '', $target = '')
		{
			...
			$page = BingMapsLibUtilsClass::GetHTTPTextElem('get', 'page');
		...
		function URLsToAnchor($page)
		{
			...
			if ($noOfMatches !== false)
			{
				...
				{ 
					$entry = $matches[0][$i];
					$url = $matches[2][$i];
					$txt = BingMapsLibMigratePHPClass::Safe_trim($matches[1][$i]);
					
					$page = BingMapsLibMigratePHPClass::Safe_str_replace($entry, '<a target="_blank" href="'.$url.'">'.$txt.'</a>', $page);
				}
			}
		...
		function compareVersion()
		{
			...
			if ($lastVersion == '')
				return self::VERSION_NEWINSTALL;

			// Compare versions
			if ($currentVersion === $lastVersion)
				return self::VERSION_UNCHANGED;
		...
		function get_pluginInfo($att = '')
		{
			if (!isset($this->pluginInfo))
			{
				...
				else
				{
					...
					for ($i = 0; ($i < 10) && BingMapsLibMigratePHPClass::Safe_strpos($basename, '/'); $i++)
		...
		function ShowDebugModes()
		{
			...
			{
				$html_dbgmsg .= '<strong>'.__('Session Debug Modes', 'bingmaps').':</strong> ';
				$comma = '';
				foreach ($debugFlagsArray as $debugMode)
				{
					$debugMode = $comma.BingMapsLibMigratePHPClass::Safe_str_replace(self::SessionDebugPrefix, '', $debugMode);
			...
			if (defined('BINGMAPSLIB_BLOCK_HTTPS'))
			{
				$html_dbgmsg .= '<strong>'.__('SSL over HTTP', 'bingmaps').":</strong> Blocked<br>\n";
			}

			BingMapsLibEscapingClass::Safe_EchoHTML($html_dbgmsg);
		...
		function InTestMode()
		{
			if (!$this->IsSessionElemSet('bingmapslib_debug_test')) return false;

			if (!function_exists('wp_get_current_user')) return false;

			return current_user_can(BINGMAPSLIB_CAPABILITY_DEVUSER);
		}
		...
		{
			$rtnVal = BingMapsLibHTTPIO::GetRequestedString($paramId, $defaultVal);
		...
		function getOptionsFromDB()
		{
			...
			{
				BingMapsLibEscapingClass::Safe_EchoHTML('CfgOptionsID must be defined<br>');
				exit;
			}

			if (!isset($this->opts['DbgOptionsID']))
			{
				BingMapsLibEscapingClass::Safe_EchoHTML('DbgOptionsID must be defined<br>');
				exit;
			}

			// Get current values from MySQL
			$currOptions = $this->ReadSettings($this->opts['CfgOptionsID']);
			if (BingMapsLibUtilsClass::IsElementSet('get', 'nodbg'))
			{
		...
		function dev_ShowTrolley()
		{
			...
			if ($this->isDbgOptionSet('Dev_ShowTrolley') || $this->IsSessionElemSet('bingmapslib_debug_trolley'))
			{
				if ($this->getDbgOption('Dev_ShowCallStack') || $this->IsSessionElemSet('bingmapslib_debug_stack'))
				{
		...
		function GetRowsPerPage()
		{
			if (isset($this->adminOptions['PageLength']))
				...
				$rowsPerPage = BINGMAPSLIB_EVENTS_PER_PAGE;
		...
		{
			$url = BingMapsLibMigratePHPClass::Safe_substr($url, BingMapsLibMigratePHPClass::Safe_strpos($url, '://')+3);
			return $url;
		}

		static function GetTimeFormat()
		{
			if (defined('BINGMAPSLIB_TIME_BOXOFFICE_FORMAT'))
				$timeFormat = BINGMAPSLIB_TIME_BOXOFFICE_FORMAT;
			else
		...
		{
			if (defined('BINGMAPSLIB_DATE_BOXOFFICE_FORMAT'))
				$dateFormat = BINGMAPSLIB_DATE_BOXOFFICE_FORMAT;
			else
		...
		{
			if (defined('BINGMAPSLIB_DATETIME_BOXOFFICE_FORMAT'))
				$dateFormat = BINGMAPSLIB_DATETIME_BOXOFFICE_FORMAT;
			else
		...
		static function GetLocalDateTime($timestamp = 0)
		{
			...
			$localTime = date(BingMapsLibDBaseClass::MYSQL_DATETIME_FORMAT, $timestamp);
			return $localTime;
		}
		...
		function DoTemplateLoop($section, $loopType, $saleRecord)
		{
			...
			switch ($loopType)
			{
				case '[startloop]':
					...
					{
						$emailContent .= $this->AddEventToTemplate($section, $ticket);
					}
					break;
		...
		function AddFieldsToTemplate($dbRecord, $mailTemplate, &$EMailSubject, &$emailContent)
		{
			...
			$posnPHP = BingMapsLibMigratePHPClass::Safe_stripos($mailTemplate, '<?php');
			if ($posnPHP !== false) $posnPHP = BingMapsLibMigratePHPClass::Safe_strpos($mailTemplate, "\n", $posnPHP);
			if ($posnPHP !== false) $posnEOL = BingMapsLibMigratePHPClass::Safe_strpos($mailTemplate, "\n", $posnPHP+1);
			if (($posnPHP !== false) && ($posnEOL !== false))
			{
				$EMailSubject = $this->AddEventToTemplate(BingMapsLibMigratePHPClass::Safe_substr($mailTemplate, $posnPHP+1, $posnEOL-$posnPHP-1), $dbRecord[0]);
				$mailTemplate = BingMapsLibMigratePHPClass::Safe_substr($mailTemplate, $posnEOL);
			}

			// Find the line with the close php entry then find the start of the line
			$posnPHP = BingMapsLibMigratePHPClass::Safe_stripos($mailTemplate, '?>');
			if ($posnPHP !== false) $posnPHP = BingMapsLibMigratePHPClass::Safe_strrpos(BingMapsLibMigratePHPClass::Safe_substr($mailTemplate, 0, $posnPHP), "\n");
			if ($posnPHP !== false) $mailTemplate = BingMapsLibMigratePHPClass::Safe_substr($mailTemplate, 0, $posnPHP);
			...
			for (; $loopCount < 10; $loopCount++)
			{
				...
				$loopStart = BingMapsLibMigratePHPClass::Safe_stripos($mailTemplate, $matches[0]);
				$loopEnd = BingMapsLibMigratePHPClass::Safe_stripos($mailTemplate, '[endloop]');
				if (($loopStart === false) || ($loopEnd === false))
					break;

				$beforeLoop = BingMapsLibMigratePHPClass::Safe_substr($mailTemplate, 0, $loopStart);

				$loopStart += BingMapsLibMigratePHPClass::Safe_strlen($matches[0]);
				$loopLen = $loopEnd - $loopStart;

				$loopSection = BingMapsLibMigratePHPClass::Safe_substr($mailTemplate, $loopStart, $loopLen);

				$loopEnd += BingMapsLibMigratePHPClass::Safe_strlen('[endloop]');
				$afterLoop = BingMapsLibMigratePHPClass::Safe_substr($mailTemplate, $loopEnd);
		...
		function DoTemplateConditionals(&$EMailTemplate, $event)
		{
			...
			$if_len = BingMapsLibMigratePHPClass::Safe_strlen($if_text);
			$ifnot_len = BingMapsLibMigratePHPClass::Safe_strlen($ifnot_text);
			$endif_len = BingMapsLibMigratePHPClass::Safe_strlen($endif_text);
			$else_len = BingMapsLibMigratePHPClass::Safe_strlen($else_text);
			...
			while(true)
			{
				...
				$nextPosn = BingMapsLibMigratePHPClass::Safe_strrpos($EMailTemplate, $if_marker, $offset);
				if ($nextPosn === false) break;
					
				// Determine type of if statement 
				if (BingMapsLibMigratePHPClass::Safe_substr($EMailTemplate, $nextPosn, $if_len) === $if_text)
				{
					$ifTagStartPosn = $nextPosn + $if_len;
					$dbCond = true;
				}
				else if (BingMapsLibMigratePHPClass::Safe_substr($EMailTemplate, $nextPosn, $ifnot_len) === $ifnot_text)
				{
				...
				{
					$offset = $nextPosn - BingMapsLibMigratePHPClass::Safe_strlen($EMailTemplate) - 1;
					continue;
				}
				
				$TemplateLen = BingMapsLibMigratePHPClass::Safe_strlen($EMailTemplate);
				
				$ifTagEndPosn = BingMapsLibMigratePHPClass::Safe_strpos($EMailTemplate, ']', $ifTagStartPosn);
				$dbField = BingMapsLibMigratePHPClass::Safe_substr($EMailTemplate, $ifTagStartPosn, $ifTagEndPosn-$ifTagStartPosn);

				// Search for end marker
				$sectionEnd = BingMapsLibMigratePHPClass::Safe_strpos($EMailTemplate, $endif_text, $nextPosn);
				if ($sectionEnd === false) 
					return __('Missing Conditional end marker', 'bingmaps');
				
				// Search forwards for else marker (assumes later else markers have been processed)
				$elseStart = BingMapsLibMigratePHPClass::Safe_strpos($EMailTemplate, $else_text, $nextPosn);
				if ($elseStart === false)
				{
				...
				if (isset($event->$dbField))
				{
					...
					{
						$dbSet = (BingMapsLibMigratePHPClass::Safe_strlen($event->$dbField) != 0);
					}
				}

				$startText = BingMapsLibMigratePHPClass::Safe_substr($EMailTemplate, 0, $nextPosn);
				$endText = BingMapsLibMigratePHPClass::Safe_substr($EMailTemplate, $endPosn, $TemplateLen);
				
				if ($dbSet == $dbCond)
				{
					// Replace the section including the conditional commands with the matched section
					$midText = BingMapsLibMigratePHPClass::Safe_substr($EMailTemplate, $ifTrueStart, $ifTrueEnd-$ifTrueStart);
				}
				else
				{
					// Replace the section with the else section (if it exists)
					$midText = BingMapsLibMigratePHPClass::Safe_substr($EMailTemplate, $ifFalseStart, $ifFalseEnd-$ifFalseStart);
				}

				$EMailTemplate = $startText.$midText.$endText;
				$changes++;
						
				$offset = 0 - BingMapsLibMigratePHPClass::Safe_strlen($midText) - BingMapsLibMigratePHPClass::Safe_strlen($endText);
			}
		...
		function AddEventToTemplate($EMailTemplate, $event)
		{
			foreach ($event as $key => $value)
			{
				...
				$EMailTemplate = BingMapsLibMigratePHPClass::Safe_str_replace($tag, $value, $EMailTemplate);
			}

			$EMailTemplate = $this->DoEmbeddedImage($EMailTemplate, 'logoimg', BINGMAPSLIB_LOGOIMAGE_OPTIONID);
		...
		function ReadTemplateFile($Filepath)
		{
			...
			{
				BingMapsLibEscapingClass::Safe_EchoHTML("Error reading $Filepath<br>\n");
				$fileContents = '';
			}
		...
		function ReadEMailTemplateFile($Filepath)
		{
			...
			$this->mailTemplate_origLen = BingMapsLibMigratePHPClass::Safe_strlen($template);
			$template = preg_replace("/^(\..*)/m", "  $1", $template);
			$this->mailTemplate_newLen = BingMapsLibMigratePHPClass::Safe_strlen($template);

			if ( ($this->mailTemplate_origLen != $this->mailTemplate_newLen)
			  && (BingMapsLibUtilsClass::IsElementSet('post', 'EMailSale_DebugEnabled')) )
			{
				BingMapsLibEscapingClass::Safe_EchoHTML("************************************************************************<br>\n");
				BingMapsLibEscapingClass::Safe_EchoHTML(__("WARNING: EMail Template contains one or mores lines with leading dots ('.')", 'bingmaps')."<br>\n");
				BingMapsLibEscapingClass::Safe_EchoHTML("************************************************************************<br><br>\n");
			}
		...
		function AddRecordToTemplate($dbRecord, $templatePath, &$EMailSubject, &$emailContent)
		{
			...
			if (BingMapsLibMigratePHPClass::Safe_strlen($mailTemplate) == 0)
				return "EMail Template Not Found ($templatePath)";

			$posnEndPHP = BingMapsLibMigratePHPClass::Safe_stripos($mailTemplate, '?>');
			$templateFooter = BingMapsLibMigratePHPClass::Safe_substr($mailTemplate, $posnEndPHP);
			$footerLines = explode("\n", $templateFooter);
			foreach ($footerLines as $footerLine)
			{
				...
				if (count($fields) > 1)
				{
					...
					{
						case 'Attach':
							$filename = BingMapsLibMigratePHPClass::Safe_trim($fields[1]);
			...
			$emailContent = apply_filters('bingmaps'.'_filter_emailbody', $emailContent, $dbRecord[0]);
			return $status;
		}
		...
		function SendEMailByTemplateID($eventRecord, $templateID, $folder, $EMailTo = '')
		{
			...
			$templateRoot = BINGMAPSLIB_UPLOADS_PATH.'/'.$folder.'/';
		...
		function OutputDebugEnd()
		{
			if ($this->debugToLog)
			{
				...
				if ($debugOutput != '')
				{
					...
					if (BingMapsLibMigratePHPClass::Safe_strpos($debugOutput, 'id="message"') !== false)
						BingMapsLibEscapingClass::Safe_EchoHTML($debugOutput);
				}
			}
		}
		...
		function ShowSQL($sql, $values = null)
		{
			...
			//if (!$this->isDbgOptionSet('Dev_ShowCaller'))
			{
				...
				$caller = BingMapsLibMigratePHPClass::Safe_str_replace("->", "::", $caller[0]);
				BingMapsLibEscapingClass::Safe_EchoHTML("SQL Caller: $caller() \n");
			}
		...
		function show_cache($results, $id='')
		{
			...
			if ($this->isDbgOptionSet('Dev_ShowCallStack'))
				BingMapsLibUtilsClass::ShowCallStack();
			BingMapsLibEscapingClass::Safe_EchoHTML("<br>Cache: $id: ".print_r($results, true)."<br>\n");
			$this->OutputDebugEnd();
		}
		...
		function WriteCommsLog($logMessage, $addTimestamp = false, $mode = BingMapsLibLogFileClass::ForAppending)
		{
			$logMessage .= "\n";
			...
			if (!isset($this->logFileObj))
			{
				...
				$this->logFileObj = new BingMapsLibLogFileClass($LogsFolder);
				$this->logFileObj->LogToFile(BINGMAPSLIB_FILENAME_COMMSLOG, '', $mode);
				$mode = BingMapsLibLogFileClass::ForAppending;
			}

			if ($addTimestamp)
				$logMessage = current_time('D, d M y H:i:s').' '.$logMessage;

			$this->logFileObj->LogToFile(BINGMAPSLIB_FILENAME_COMMSLOG, $logMessage, $mode);
		}

		function ClearCommsLog()
		{
			return $this->WriteCommsLog('Log Cleared', true, BingMapsLibLogFileClass::ForWriting);
		}

		function GetCommsLog()
		{
			$LogsFolder = ABSPATH.$this->getOption('LogsFolderPath');
			$logsPath = $LogsFolder.'/'.BINGMAPSLIB_FILENAME_COMMSLOG;
		...
		function GetSessionID()
		{
			if (!isset($this->sessionCookieID))
			{
				...
				if (!$this->tableExists($this->DBTables->Sessions))
				{
				...
				{
					$this->PurgeSessions();
					if ($this->ReadSession($this->sessionCookieID) === null)
					{
						$addSession = true;
					}
				}
				...
				if ($addSession)
				{
					...
					$sessionExpires = date(BingMapsLibDBaseClass::MYSQL_DATETIME_FORMAT, current_time( 'timestamp' ) + BINGMAPSLIB_SESSION_TIMEOUT);
					$sessionValSer = serialize(array());
		...
		function PurgeSessions()
		{
			...
			$currDateTime = date(BingMapsLibDBaseClass::MYSQL_DATETIME_FORMAT, current_time( 'timestamp' ));
		...
		function ReadSession()
		{
			...
			return unserialize($sessionEntries[0]->sessionVal);
		}
		...
		function WriteSession()
		{
			...
			$sessionValSer = BingMapsLibMigratePHPClass::Safe_str_replace('"', '\"', serialize($this->sessionVal));
