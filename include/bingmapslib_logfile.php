...
if (!class_exists('BingMapsLibLogFileClass'))
{
	class BingMapsLibLogFileClass // Define class
	{
		const ForReading = 1;
		const ForWriting = 2;
		...
		function __construct($LogsFolderPath = '')
		{
			...
			if (BingMapsLibMigratePHPClass::Safe_substr($LogsFolderPath, -1) === '/')
			{
				$LogsFolderPath = BingMapsLibMigratePHPClass::Safe_substr($LogsFolderPath, 0, -1);
			}

			if (!is_dir($LogsFolderPath))
			{
				mkdir($LogsFolderPath, BINGMAPSLIB_LOGFOLDER_PERMS, true);
			}
		...
		{
			$Filepath = BingMapsLibMigratePHPClass::Safe_str_replace("\\", "/", $Filename);

			for ($i = 1; $i <= 2; $i++)
			{
				$firstChar = BingMapsLibMigratePHPClass::Safe_substr($Filepath, 0, 1);
				if (BingMapsLibMigratePHPClass::Safe_strpos($Filepath, ':') || ($firstChar == '/'))
				{
				...
				else if ($i == 1)
				{
					...
					if (BingMapsLibMigratePHPClass::Safe_strlen($this->LogsFolderPath) > 0)
		...
		function StampedLogToFile($Filename, $LogLine, $OpenMode = self::ForAppending, $LogHeader = '')
		{
			...
			$LogStamp .= 'Log Length: '.BingMapsLibMigratePHPClass::Safe_strlen($LogLine)."\n";
			$LogStamp .= "\n";
		...
		static function LogToFileAbs($Filepath, $LogLine, $OpenMode = self::ForAppending, $LogHeader = '')
		{
			...
			if ($logFile != 0)
			{
				...
				if ($LogHeader !== '')
				{
					...
					if (ftell($logFile)	=== 0)
						fwrite($logFile, $LogHeader, BingMapsLibMigratePHPClass::Safe_strlen($LogHeader));
				}

				fwrite($logFile, $LogLine, BingMapsLibMigratePHPClass::Safe_strlen($LogLine));
				fclose($logFile);
			...
			{
				BingMapsLibEscapingClass::Safe_EchoHTML("Error writing to ".$Filepath."<br>\n");
				$rtnStatus = false;
			}
		...
		function DumpToFile($Filename, $dataId, $dataToDump)
		{
			...
			$dataLen = BingMapsLibMigratePHPClass::Safe_strlen($dataToDump);
			...
			for ($i=0;;$i++)
			{
				...
				$nextChar = BingMapsLibMigratePHPClass::Safe_substr($dataToDump, $i, 1);
				if ($i < $dataLen)
				{
		...
		function AddToTestLog($LogLine)
		{
			...
			self::LogToFileAbs($Filepath, 'Log Time/Date:'.BingMapsLibDBaseClass::GetLocalDateTime()."\n");
			//self::LogToFileAbs($Filepath, 'Request URL:'.BingMapsLibUtilsClass::GetHTTPTextElem('server', 'REQUEST_URI')."\n");
