...
if (!class_exists('BingMapsLibTableClass')) 
{
	...
	if (!defined('BINGMAPSLIB_DISABLE_POSTCONTROLS')) 
		define('BINGMAPSLIB_DISABLE_POSTCONTROLS', true);
	
	class BingMapsLibTableUtilsClass // Define class
	{
		static function GetCurrentPage()
		{ 
			$currentPage = BingMapsLibUtilsClass::GetHTTPInteger('request', 'paged', 1);
			$currentPage = BingMapsLibUtilsClass::GetHTTPInteger('get', 'paged', $currentPage);
			return $currentPage;
		}
	...
	class BingMapsLibTableClass // Define class
	{
		...
		const BINGMAPSLIB_EVENTS_UNPAGED = -1;
		...
		var $showOptionsID = 0;
		var $hiddenRowClass  = 'bingmapslib_hiderow';
		var $visibleRowClass = '';
		...
		function __construct($newTableType = self::TABLETYPE_HTML) //constructor
		{
			...
			switch ($this->tableType)
			{
				...
				default:
					...
					BingMapsLibEscapingClass::Safe_EchoHTML("<strong><br>Invalid table type ($newTableType) ".get_class($this)." class<br></strong>\n");
					die;
					break;
			}
			...
			$this->moreText = __('Show', 'bingmaps');
			$this->lessText = __('Hide', 'bingmaps');
		...
		function SetRowsPerPage($rowsPerPage)
		{
			...
			$this->currentPage = BingMapsLibTableUtilsClass::GetCurrentPage();			
		}
		...
		function EnableListHeaders()
		{
			...
			if (isset($this->bulkActions))
			{
				...
				$columns = array_merge(array('eventCb' => '<input name="checkall" id="checkall" type="checkbox"  onClick="BingMapsLib_updateCheckboxes(this)" />'), $columns); 
			}
		...
		function AddCheckBoxToTable($result, $columnDef, $checked=false, $col=0, $checkedValue='checked', $label='', $newRow = false)
		{
			...
			if (BingMapsLibMigratePHPClass::Safe_substr($inputName, -2) != '[]')
			{
		...
		function GetInputHTML($inputName, $maxlength, $value, $extraParams='')
		{
			...
			{
				if (BingMapsLibMigratePHPClass::Safe_strpos($extraParams, 'class="') !== false)
				{
					$extraParams = BingMapsLibMigratePHPClass::Safe_str_replace('class="', 'class="bingmapslib_PostVal ', $extraParams);
				}
				else
				{
					$extraParams .= ' class="bingmapslib_PostVal"';					
				}
			}
			else
			{
			...
			if ($this->allowHiddenTags)
			{
				...
				if ($this->tableUsesSerializedPost)
				{
					$baseParams = 'class="bingmapslib_PostVal"';					
				}
				else
				{
		...
		function AddSelectToTable($result, $columnDef, $options, $value='', $col=0, $newRow = false)
		{
			...
			if ($this->tableUsesSerializedPost)
			{
				$params = ' class="bingmapslib_PostVal"';
			}
			else
			{
				$params = " name=$inputName";
		...
		function AddShowOrHideButtonToTable($result, $tableId, $rowId, $content, $col=0, $newRow = false)
		{
			...
			$content = '<a id="'.$moreName.'" class="button-secondary" onClick="BingMapsLib_HideOrShowRows(\''.$moreName.'\', \''.$rowId.'\')">'.$content.'</a>';
			$this->AddToTable($result, $content, $col, $newRow);
		}
		...
		function AddToTable($result, $content, $col=0, $newRow = false)
		{
			...
			{
			if (!isset($content) || (BingMapsLibMigratePHPClass::Safe_strlen($content) == 0)) return;
			}
		...
		{
			return 'bingmapslib_ClickHeader(this)';
		}
		...
		function Output_ColHeader()
		{
			...
			foreach ($this->mergedColumns as $id => $html_col)
			{
				if ($addSeparator)
				{
					...
					BingMapsLibEscapingClass::Safe_EchoHTML($html_cell."\n");
				}
		...
		function ColumnHeaders($atTop = true)
		{
			...
			if ($atTop)
			{
				...
				BingMapsLibEscapingClass::Safe_EchoHTML("<thead>\n");
				BingMapsLibEscapingClass::Safe_EchoHTML("<tr>\n");
			...
			else
			{
				...
				BingMapsLibEscapingClass::Safe_EchoHTML("<tfoot>\n");
				BingMapsLibEscapingClass::Safe_EchoHTML("<tr>\n");
				$this->Output_ColHeader();
				BingMapsLibEscapingClass::Safe_EchoHTML("</tr>\n");
				BingMapsLibEscapingClass::Safe_EchoHTML("</tfoot>\n");
				BingMapsLibEscapingClass::Safe_EchoHTML("<tbody>\n");
			}
		}
		...
		function Header()
		{
			switch ($this->tableType)
			{
				case self::TABLETYPE_HTML:
					if ($this->divClass)
						BingMapsLibEscapingClass::Safe_EchoHTML("<div class=$this->divClass>\n");
					...
					BingMapsLibEscapingClass::Safe_EchoHTML($html_tab);
		...
		function Footer()
		{
			...
			{
				case self::TABLETYPE_HTML:
					BingMapsLibEscapingClass::Safe_EchoHTML("</tbody></table>\n");
					if ($this->divClass)
		...
		{	
			...
			$bulkActions = __('Bulk Actions', 'bingmaps');
			
			$onclickParam = "onclick=\"return BingMapsLib_confirmBulkAction(this, '$buttonId')\"";
			...
			$output .= "</select>\n"; 
			$output .= "<input type='submit' name='doaction$ctrlPosn' id='doaction$ctrlPosn' $onclickParam class='button-secondary action' value=".__('Apply', 'bingmaps')."  />\n";
			$output .= "</div>\n"; 
...
lessText = '{$this->lessText}';
			...
			";
			BingMapsLibEscapingClass::Safe_EchoScript($js_more);
		}
...
var confirmActionsArray = new Array(";
			...
			BingMapsLibEscapingClass::Safe_EchoScript($html_bulkActions);
		}
		
		function GetCurrentURL() 
		{			
			$currentURL = BingMapsLibUtilsClass::GetPageURL();
		...
		{			
			...
			$page_links[] = sprintf( "<a class='%s' title='%s' %s>%s</a>",
				...
				$disable_first === '' ? __('Go to the first page', 'bingmaps') : '',
			...
			$page_links[] = sprintf( "<a class='%s' title='%s' %s>%s</a>",
				...
				$disable_first === '' ? __('Go to the previous page', 'bingmaps') : '',
			...
			{
				$onKeyCodeHandler = ' onkeypress="BingMapsLib_SubmitOnReturnKey(this, event);" ';
				$html_current_page = sprintf( "<input class='current-page' title='%s' type='text' name='%s' value='%s' $onKeyCodeHandler size='%d' />",
					__( 'Current page', 'bingmaps'),
					'paged',
					$this->currentPage,
					BingMapsLibMigratePHPClass::Safe_strlen( $totalPages )
				);
			}

			$html_total_pages = sprintf( "<span class='total-pages'>%s</span>", number_format_i18n( $totalPages ) );
			$page_links[] = '<span class="paging-input">' . sprintf('%1$s '.__('of', 'bingmaps').' %2$s', $html_current_page, $html_total_pages ) . '</span>';

			$page_links[] = sprintf( "<a class='%s' title='%s' %s>%s</a>",
				'next-page button' . $disable_last,
				$disable_last === '' ? __('Go to the next page', 'bingmaps') : '',
			...
			$page_links[] = sprintf( "<a class='%s' title='%s' %s>%s</a>",
				...
				$disable_last === '' ? __('Go to the last page', 'bingmaps') : '',
		...
		function ShowPageControls($which = 'top')
		{
			...
			if ($pageControls != '') 
			{
				...
				BingMapsLibEscapingClass::Safe_EchoHTML($html_bulkActions);
			}
		}
			...
			for ($row = 1; $row <= $this->currRow; $row++)
			{
				...
				switch ($this->tableType)
				{
					case self::TABLETYPE_HTML:
						if (isset($this->rowAttr[$row]) && ($this->rowAttr[$row] != ''))
						{
							...
							BingMapsLibEscapingClass::Safe_EchoHTML($html_row);
						}
						else
							BingMapsLibEscapingClass::Safe_EchoHTML("<tr>\n");
						break;
				...
				for ($col = 1; $col <= $this->maxCol; $col++)
				{
					...
					switch ($this->tableType)
					{
						case self::TABLETYPE_HTML:
							...
							BingMapsLibEscapingClass::Safe_EchoHTML($html_table);
							break;
						case self::TABLETYPE_RTF:
							if ($col > 1) BingMapsLibEscapingClass::Safe_EchoAttr('\tab ');
						case self::TABLETYPE_TEXT:
						default:
							break;
					}
					$html_tableContents = isset($this->tableContents[$row][$col]) ? $this->tableContents[$row][$col] : "";
					$html_tableContents = BingMapsLibMigratePHPClass::Safe_trim($html_tableContents);
					if (BingMapsLibMigratePHPClass::Safe_strlen($html_tableContents) == 0) $html_tableContents = "&nbsp;";
					...
					BingMapsLibEscapingClass::Safe_EchoHTML($html_tableContents);
				...
				switch ($this->tableType)
				{
					case self::TABLETYPE_HTML:
						...
						break;
					case self::TABLETYPE_RTF:
						BingMapsLibEscapingClass::Safe_EchoAttr('\par '."\n");
						break;
					case self::TABLETYPE_TEXT:
					default:
						BingMapsLibEscapingClass::Safe_EchoAttr("\n");
						break;
				}
			}
