...
{
	if (!defined('BINGMAPSLIB_PHPFOLDER_PERMS'))
		define('BINGMAPSLIB_PHPFOLDER_PERMS', 0755);

	if (!defined('BINGMAPSLIB_PHPFILE_PERMS'))
		define('BINGMAPSLIB_PHPFILE_PERMS', 0644);

	if (!defined('BINGMAPSLIB_LOGFOLDER_PERMS'))
		define('BINGMAPSLIB_LOGFOLDER_PERMS', 0700);

	if (!defined('BINGMAPSLIB_CALLBACK_BASENAME'))
		define('BINGMAPSLIB_CALLBACK_BASENAME', basename(dirname(dirname(__FILE__))).'_callback');
	...
	class BingMapsLibUtilsClass // Define class
	{
		static function GetSiteID()
		{
			...
			$slashPosn = BingMapsLibMigratePHPClass::Safe_strrpos($siteURL, '/');
			$siteURL = BingMapsLibMigratePHPClass::Safe_substr($siteURL, $slashPosn+1);
		...
		static function GetHTTPTextElem($reqArray, $elementId = 'undef', $defaultValue = '')
		{
			...
			$str = BingMapsLibMigratePHPClass::Safe_stripslashes($str);
			return $str;
		}
		...
		static function bingmapslib_filterHTMLStyles($styles)
		{
		...
		{
			global $bingmapslib_allowedProtocols;

			if (!is_array($bingmapslib_allowedProtocols))
			{
				$bingmapslib_allowedProtocols = wp_allowed_protocols();
				$bingmapslib_allowedProtocols[] = 'data';
			}
			
			return $bingmapslib_allowedProtocols;
			
		}

		static function GetAllowedHTMLTags($extraTags = array())
		{
			global $bingmapslib_allowedHTML;

			if (!is_array($bingmapslib_allowedHTML))
			{
				add_filter( 'safe_style_css', array('BingMapsLibUtilsClass', 'bingmapslib_filterHTMLStyles'));
				...
				$bingmapslib_allowedHTML = $kses_allowed;
			}

			return $bingmapslib_allowedHTML;
		}
		...
		static function GetHTTPJSONEncodedElem($reqArray, $elementId)
		{
			...
			foreach($postVarsArray as $postKey => $postVal)
			{
				...
				BingMapsLibUtilsClass::SetElement('post', $postKey, $postVal);
				BingMapsLibUtilsClass::SetElement('request', $postKey, $postVal);
			}
		...
		static function GetHTTPInteger($reqArray, $elementId = 'undef', $defaultValue = null)
		{
			...
			$strval = BingMapsLibMigratePHPClass::Safe_trim(self::GetArrayElement($reqArray, $elementId, $defaultValue));
			$rtnval = (int)$strval;
			if ($rtnval != $strval)
			{
				return $defaultValue;
		...
		{
		    return (bool)BingMapsLibMigratePHPClass::Safe_strtotime($myDateString);
		}

		static function GetDateTimestamp($myDateString)
		{
			$spPosn = BingMapsLibMigratePHPClass::Safe_strpos($myDateString, " ");
			$ts = BingMapsLibMigratePHPClass::Safe_strtotime(BingMapsLibMigratePHPClass::Safe_substr($myDateString, 0, $spPosn));
			return $ts;
		}
		...
		static function GetPageBaseURL()
		{
			...
			$posnParams = BingMapsLibMigratePHPClass::Safe_strpos($pageURL,'?');
			if ($posnParams !== false)
				$pageURL = BingMapsLibMigratePHPClass::Safe_substr($pageURL, 0, $posnParams);
			return $pageURL;
		}
		...
		{
			return self::GetPageURL().'&'.BINGMAPSLIB_CALLBACK_BASENAME."={$callbackFilename}";
		}
		...
		{
		     $length = BingMapsLibMigratePHPClass::Safe_strlen($needle);
		     return (BingMapsLibMigratePHPClass::Safe_substr($haystack, 0, $length) === $needle);
		}

		static function recurse_copy($src, $dst, $perm=BINGMAPSLIB_PHPFOLDER_PERMS)
		{
			$rtnStatus = true;
		...
		static function UndefinedFuncCallError($classObj, $funcName)
		{
			...
			BingMapsLibEscapingClass::Safe_EchoHTML("<strong><br>function $funcName() must be defined in $classId class<br></strong>\n");
			die;
		}
		...
		static function UserMessage($msg, $isError = true, $msgClass = '')
		{
			...
			BingMapsLibEscapingClass::Safe_EchoHTML('<div id="message" class="'.$msgClass.'"><p>'.$msg.'</p></div>');
		}
